...
## 4.8 Electron renderers: what the semantic channel can and cannot confirm

The VS Code-family adapter (§3.3a in ARCHITECTURE) proved that an editor's *own commands* are a
usable control channel where synthetic input is not. That raised the obvious question for every
remaining Electron gap: which of them have an equivalent channel, and which are simply not
observable? Confirmability is the deciding property, because an operation SpaceO cannot observe
is one it must not report as success.

The table below separates two very different kinds of claim. **Contract** rows follow from the
VS Code extension API surface itself — an API that does not exist cannot be called, and no
amount of testing changes that. **Measured** rows are the earlier hands-on results already
recorded in this document and in SPAO-179. Rows marked *unverified* are implemented against the
contract but have not yet been exercised against a live Cursor instance.

| capability | channel | confirmable? |
|---|---|---|
| vertical scroll, single pane | `editorScroll` | **yes** — visible-range delta (measured, SPAO-179) |
| vertical scroll, split pane | `TextEditor.revealRange` on the addressed editor | **yes** — per-editor visible-range delta (contract; unverified live) |
| horizontal scroll | `revealRange` at a character column | **no** — see below (contract) |
| typing / keys | per-PID events + `TextDocument.version` and selection read-back | **yes**, as an after-the-fact check (contract; unverified live) |
| text selection | `TextEditor.selection` set, then read back | **yes** — the editor reports what it adopted (contract; unverified live) |
| hover | — | **no channel** (contract) |
| true press-drag-release | — | **no channel** (contract) |

**Horizontal scrolling cannot be confirmed.** `TextEditor` exposes `visibleRanges`, which are
line ranges; there is no horizontal scroll offset anywhere on the type. So the viewport can be
*moved* horizontally (reveal a character position on a visible line) but not *observed*
horizontally. SpaceO therefore performs the reveal and reports it through the established
unconfirmed-delivery warning rather than claiming an effect it cannot see. The adapter keeps a
per-document estimate of the column it last revealed purely so a relative request has something
to move from; that estimate is a model of the viewport, not a reading of it, and it is labelled
as such in both the code and the response.

**Hover has no channel.** VS Code exposes `editor.action.showHover`, but it acts on the cursor
position rather than an arbitrary point, and it reports nothing about what appeared — so the two
things a hover is for (reveal UI at a point, then see what was revealed) are both missing. A
synthetic `mouseMoved` remains the only option and remains unconfirmable, exactly as it is today.

**Drag has no channel, but its main use does.** There is no API to synthesise a press-drag-release
inside the renderer. Most agent drags on a code editor are text selection, and that *is*
expressible: setting `TextEditor.selection` and reading back what the editor adopted is strictly
better evidence than a drag would have produced, because it confirms the resulting selection
rather than the gesture. Drags whose purpose is not selection — reordering tabs, resizing panes,
dragging onto a canvas — have no equivalent and stay unsupported rather than silently attempted.

The negative results here are recorded so they are not re-litigated: the absence of a horizontal
viewport offset, of a point-addressed hover, and of a renderer drag primitive are properties of
the published extension API, not gaps in the current implementation.
...
## 5. Attention leaks that remain, and what was tried (2026-09-17)

Recorded so they are not re-investigated.

- **Notifications.** Agent apps still post banners to the user's display. No public API silences
  another app's notifications; a Focus filter is the supported route. `spaceo setup` now lists
  the apps SpaceO has launched so the user can allow everything except those in a Focus.
- **Dock and ⌘-Tab.** A background app with windows on a virtual display still appears in both.
  Hiding it would require the app to declare `LSUIElement`, which SpaceO must not change.
  Session titles in the Viewer reduce confusion when the user does see them.
- **Audio.** Chromium honours `--mute-audio`; SpaceO passes it for a managed browser launch when
  `mute_audio` is requested. There is no per-process mute for other apps without private
  CoreAudio APIs, so they remain audible and the limitation is documented.
