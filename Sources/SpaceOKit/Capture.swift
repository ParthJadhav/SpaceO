...
public enum Capture {
    ...
    /// One session's tile of a shared agent display.
    ///
    ...
    public static func region(_ stage: Stage, _ rect: CGRect) async throws -> CGImage {
        guard CGPreflightScreenCaptureAccess() else { throw SpaceOError.screenRecordingDenied }
        let content = try await shareableContent()
        guard let target = content.displays.first(where: { $0.displayID == stage.displayID }) else {
            throw SpaceOError.captureFailed("display \(stage.displayID) is not shareable")
        ...
        let dimensions = try validatedDimensions(width: local.width, height: local.height)
        config.width = dimensions.width
        config.height = dimensions.height
        config.showsCursor = false
        config.captureResolution = .best
    ...
    public static func window(_ window: WindowRef) async throws -> CGImage {
        guard CGPreflightScreenCaptureAccess() else { throw SpaceOError.screenRecordingDenied }
        let content = try await shareableContent()
        ...
        let config = SCStreamConfiguration()
        let dimensions = try validatedDimensions(
            width: target.frame.width, height: target.frame.height, scale: 2)
        config.width = dimensions.width
        config.height = dimensions.height
        config.showsCursor = false
        config.captureResolution = .best
