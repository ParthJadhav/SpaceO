...
4. Confirm `spaceo version`, run `spaceo doctor`, and restart MCP clients. Doctor's
   **MCP clients** section shows the version of the binary each client launches; a `STALE`
   entry still points at an old copy.

If the old daemon is still running, `spaceo daemon restart --operator` replaces it with the new
build. This works even against daemons too old to drain: it waits (bounded, with progress) until
the old daemon has no live sessions, stops it, and starts the new one. `--now` stops it
immediately.
...
explicit release-owner approval required by the [release policy](RELEASE_POLICY.md).
...
2. runs the safe test suite, optimized build, and MCP black-box smoke test;
...
`release-publication` environment, the qualifier must download that exact candidate,
run:
