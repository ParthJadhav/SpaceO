...
APP_ICON="$REPOSITORY_ROOT/Assets/Brand/SpaceO.icns"
BRAND_MARK="$REPOSITORY_ROOT/Assets/Brand/spaceo-logo.png"
...
[[ -f "$APP_ICON" ]] || {
    echo "error: missing Viewer icon at $APP_ICON; run scripts/build-brand-assets.sh" >&2
    exit 1
}
[[ -f "$BRAND_MARK" ]] || {
    echo "error: missing Viewer brand mark at $BRAND_MARK" >&2
    exit 1
}
[[ "$VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || {
...
cp "$APP_ICON" "$APP/Contents/Resources/SpaceO.icns"
cp "$BRAND_MARK" "$APP/Contents/Resources/spaceo-logo.png"
...
<dict>
    ...
    <key>CFBundleIconFile</key>        <string>SpaceO.icns</string>
