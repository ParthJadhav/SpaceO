...
- A watcher that cannot be *built* — `AXObserverCreate` refusing because Accessibility is off, or
  because an adopted pid is not AX-registered yet — takes both mechanisms above with it, since
  the notification handler and the periodic sweep are the same object. The reason is recorded on
  the session (`watcherCreationFailures`) rather than left as an absent dictionary key, the audit
  reports the app as unwatched, and the janitor pass covers it directly: it retries construction
  each tick and full-bounds re-parks that app's escaped windows itself. Without that, an app with
  no watcher contributes to no health signal at all and reads as perfectly contained.
...
- `PasteboardGuard` — bounded snapshot/restore for tests and diagnostics only. It is deliberately
  **not** a production copy/cut guard: `NSPasteboard` has no compare-and-swap, so no
  ...
  check and its write. Production key delivery never synthesises Command-C, Command-X or
  Command-V against the shared pasteboard (`KeyCombo.requireClipboardSafeRoute`).
- `SessionClipboard` / `ClipboardRoute` — the per-session clipboard broker (SPAO-143). Each
  `AgentSession` owns a 1 MiB text buffer that `clipboard.set`/`clipboard.get` read and write.
  The `key` command intercepts plain ⌘C/⌘X/⌘V: copy and cut read the focused element's
  `AXSelectedText` (or the page selection through DevTools) into the buffer, cut then sends
  Delete and reports whether the selection changed; paste inserts the buffer through
  `Input.insertText` for Chromium, by typing for native apps, or by appending to a settable
  focused field's value as a last resort, and reports `inserted_via`. `NSPasteboard.general` is
  never read or written on any agent route; a test greps the broker source for it. The Viewer's
  explicit "Copy from Session" is the one place the user's pasteboard is written, on the
  human's own click.
- `EventBus` — daemon-wide ring of `DaemonEvent`s (4096) that `events.poll` and the streaming
  `events.subscribe` connection replay from a sequence number, redacted per lease exactly like
  `session.list`. The Viewer consumes it instead of polling; `spaceo events --follow` tails it.
...
- That epoch is also why a transition **schedules** its queue cleanup instead of awaiting it. Both
  setters run on the main actor, and the drain can sit behind a backlog whose worst case is
  measured in seconds — `AX.element(at:in:)`'s one-second messaging timeout per candidate window,
  focus settle sleeps, pointer pacing. Blocking on it bought nothing the epoch does not already
  guarantee, and charged that budget to the main thread at exactly the moments the UI must stay
  responsive: the emergency-exit chord, a display change mid-drag. Scheduling also moves the
  post-drain route restoration *onto* the delivery queue, where it is ordered against the events
  it is cancelling rather than racing a newly admitted down.
